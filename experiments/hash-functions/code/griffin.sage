"""Educational SageMath implementation of the Griffin permutation and sponge.

This is based on the implementation supplied with the original notes and is
intended for study and reproducible experimentation.  Production proof systems
must use a reviewed implementation and an exact parameter set required by the
protocol; changing field, width, capacity, rounds, padding or serialization
...
def get_matrix(p, t):
    # For larger widths, construct the specified block matrix directly over
    # the finite field.  Using Sage matrices keeps modular arithmetic explicit.
    Fp = FiniteField(p)
    if t == 3:
...
def griffin_sponge(parameters, input_sequence, output_length):
    ...
    # Work on a copy: hashing must not mutate the caller's input list.
    ...
    while absorb_index < len(padded_input):
        for i in range(0, rate):
            state[i, 0] += Fp(padded_input[absorb_index])
            absorb_index += 1
...
def demo():
    """Print deterministic outputs that can be retained as regression vectors."""

    Fp = GF(p)
    inputs = [
        [Fp(0) for _ in range(8)],
        [Fp(1) for _ in range(8)],
        [Fp(i) for i in range(8)],
    ]
    for sequence in inputs:
        before = list(sequence)
        output = griffin_sponge(parameters, sequence, 4)
        assert sequence == before  # regression check for the former mutation bug
        print([int(element) for element in output])
